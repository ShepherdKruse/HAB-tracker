// Include the RFM69 and SPI libraries:
#include <OneWire.h>
#include <SPI.h>
#include <NMEAGPS.h>
#include <GPSport.h>
#define LED           4 // LED positive pin
#define GND           3 // LED ground pin

long readVcc() {
  // Read 1.1V reference against AVcc
  // set the reference to Vcc and the measurement to the internal 1.1V reference
  #if defined(__AVR_ATmega32U4__) || defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)
    ADMUX = _BV(REFS0) | _BV(MUX4) | _BV(MUX3) | _BV(MUX2) | _BV(MUX1);
  #elif defined (__AVR_ATtiny24__) || defined(__AVR_ATtiny44__) || defined(__AVR_ATtiny84__)
    ADMUX = _BV(MUX5) | _BV(MUX0);
  #elif defined (__AVR_ATtiny25__) || defined(__AVR_ATtiny45__) || defined(__AVR_ATtiny85__)
    ADMUX = _BV(MUX3) | _BV(MUX2);
  #else
    ADMUX = _BV(REFS0) | _BV(MUX3) | _BV(MUX2) | _BV(MUX1);
  #endif  

  delay(2); // Wait for Vref to settle
  ADCSRA |= _BV(ADSC); // Start conversion
  while (bit_is_set(ADCSRA,ADSC)); // measuring

  uint8_t low  = ADCL; // must read ADCL first - it then locks ADCH  
  uint8_t high = ADCH; // unlocks both

  long result = (high<<8) | low;

  result = 1125300L / result; // Calculate Vcc (in mV); 1125300 = 1.1*1023*1000
  return result; // Vcc in millivolts
}

NMEAGPS  gps; // This parses the GPS characters
gps_fix  fix; // This holds on to the latest values

 
 

void setup()
{

  // Open a serial port so we can send keystrokes to the module:
pinMode(LED,OUTPUT);
  digitalWrite(LED,LOW);
  pinMode(GND,OUTPUT);
  digitalWrite(GND,LOW);
Blink(LED,10);

  Serial.begin(1200);
  gpsPort.begin(9600);

}

void loop()
{
  float latitude = (fix.latitude());
  float longitude = (fix.longitude());
   while (gps.available( gpsPort )) {
    fix = gps.read();
   if (fix.valid.location){
    if(fix.valid.altitude){
    String Location;
     Location += String(latitude, 6);
    Location += F(" , ");
    Location += String(longitude , 6);
    Location += F(" , ");
    Location += String(fix.altitude());
    Location += String(" , ");
    Location += String(fix.speed());
    Location += F(" , ");
    Location += String(fix.satellites);
    Location += F(" , ");
    Location += String(readVcc());
    Serial.print(Location);
    uint8_t GPSoff[] = {0xB5, 0x62, 0x06, 0x04, 0x04, 0x00, 0x00, 0x00,0x08, 0x00, 0x16, 0x74};
    delay(60000);
    uint8_t GPSon[] = {0xB5, 0x62, 0x06, 0x04, 0x04, 0x00, 0x00, 0x00,0x09, 0x00, 0x17, 0x76};
    }
   }
   else 
   {
    Blink(LED,10);
  String Initializing;
    Initializing += F("Initializing ");
    Initializing += String(fix.satellites);
    Initializing += F(" , ");
    Initializing += String(readVcc());
   Serial.print(Initializing);
   delay(60000);
   }
   }         
}

void Blink(byte PIN, int DELAY_MS)
// Blink an LED for a given number of ms
{
  digitalWrite(PIN,HIGH);
  delay(DELAY_MS);
  digitalWrite(PIN,LOW);
}

